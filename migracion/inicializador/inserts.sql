SET FOREIGN_KEY_CHECKS = 0;

INSERT INTO
    depto_muni (
        codigo,
        departamento,
        municipio,
        lugar
    )
VALUES (
        '0101',
        'Guatemala',
        'Guatemala',
        'Guatemala, Guatemala'
    ),
    (
        '0102',
        'Guatemala',
        'Santa Catarina Pinula',
        'Santa Catarina Pinula, Guatemala'
    ),
    (
        '0103',
        'Guatemala',
        'San José Pinula',
        'San José Pinula, Guatemala'
    ),
    (
        '0104',
        'Guatemala',
        'San José del Golfo',
        'San José del Golfo, Guatemala'
    ),
    (
        '0105',
        'Guatemala',
        'Palencia',
        'Palencia, Guatemala'
    ),
    (
        '0106',
        'Guatemala',
        'Chinautla',
        'Chinautla, Guatemala'
    ),
    (
        '0107',
        'Guatemala',
        'San Pedro Ayampuc',
        'San Pedro Ayampuc, Guatemala'
    ),
    (
        '0108',
        'Guatemala',
        'Mixco',
        'Mixco, Guatemala'
    ),
    (
        '0109',
        'Guatemala',
        'San Pedro Sacatepéquez',
        'San Pedro Sacatepéquez, Guatemala'
    ),
    (
        '0110',
        'Guatemala',
        'San Juan Sacatepéquez',
        'San Juan Sacatepéquez, Guatemala'
    ),
    (
        '0111',
        'Guatemala',
        'San Raymundo',
        'San Raymundo, Guatemala'
    ),
    (
        '0112',
        'Guatemala',
        'Chuarrancho',
        'Chuarrancho, Guatemala'
    ),
    (
        '0113',
        'Guatemala',
        'Fraijanes',
        'Fraijanes, Guatemala'
    ),
    (
        '0114',
        'Guatemala',
        'Amatitlán',
        'Amatitlán, Guatemala'
    ),
    (
        '0115',
        'Guatemala',
        'Villa Nueva',
        'Villa Nueva, Guatemala'
    ),
    (
        '0116',
        'Guatemala',
        'Villa Canales',
        'Villa Canales, Guatemala'
    ),
    (
        '0117',
        'Guatemala',
        'San Miguel Petapa',
        'San Miguel Petapa, Guatemala'
    ),
    (
        '0201',
        'El Progreso',
        'Guastatoya',
        'Guastatoya, El Progreso'
    ),
    (
        '0202',
        'El Progreso',
        'Morazán',
        'Morazán, El Progreso'
    ),
    (
        '0203',
        'El Progreso',
        'San Agustín Acasaguastlán',
        'San Agustín Acasaguastlán, El Progreso'
    ),
    (
        '0204',
        'El Progreso',
        'San Cristóbal Acasaguastlán',
        'San Cristóbal Acasaguastlán, El Progreso'
    ),
    (
        '0205',
        'El Progreso',
        'El Jícaro',
        'El Jícaro, El Progreso'
    ),
    (
        '0206',
        'El Progreso',
        'Sansare',
        'Sansare, El Progreso'
    ),
    (
        '0207',
        'El Progreso',
        'Sanarate',
        'Sanarate, El Progreso'
    ),
    (
        '0208',
        'El Progreso',
        'San Antonio La Paz',
        'San Antonio La Paz, El Progreso'
    ),
    (
        '0301',
        'Sacatepéquez',
        'Antigua Guatemala',
        'Antigua Guatemala, Sacatepéquez'
    ),
    (
        '0302',
        'Sacatepéquez',
        'Jocotenango',
        'Jocotenango, Sacatepéquez'
    ),
    (
        '0303',
        'Sacatepéquez',
        'Pastores',
        'Pastores, Sacatepéquez'
    ),
    (
        '0304',
        'Sacatepéquez',
        'Sumpango',
        'Sumpango, Sacatepéquez'
    ),
    (
        '0305',
        'Sacatepéquez',
        'Santo Domingo Xenacoj',
        'Santo Domingo Xenacoj, Sacatepéquez'
    ),
    (
        '0306',
        'Sacatepéquez',
        'Santiago Sacatepéquez',
        'Santiago Sacatepéquez, Sacatepéquez'
    ),
    (
        '0307',
        'Sacatepéquez',
        'San Bartolomé Milpas Altas',
        'San Bartolomé Milpas Altas, Sacatepéquez'
    ),
    (
        '0308',
        'Sacatepéquez',
        'San Lucas Sacatepéquez',
        'San Lucas Sacatepéquez, Sacatepéquez'
    ),
    (
        '0309',
        'Sacatepéquez',
        'Santa Lucía Milpas Altas',
        'Santa Lucía Milpas Altas, Sacatepéquez'
    ),
    (
        '0310',
        'Sacatepéquez',
        'Magdalena Milpas Altas',
        'Magdalena Milpas Altas, Sacatepéquez'
    ),
    (
        '0311',
        'Sacatepéquez',
        'Santa María de Jesús',
        'Santa María de Jesús, Sacatepéquez'
    ),
    (
        '0312',
        'Sacatepéquez',
        'Ciudad Vieja',
        'Ciudad Vieja, Sacatepéquez'
    ),
    (
        '0313',
        'Sacatepéquez',
        'San Miguel Dueñas',
        'San Miguel Dueñas, Sacatepéquez'
    ),
    (
        '0314',
        'Sacatepéquez',
        'San Juan Alotenango',
        'San Juan Alotenango, Sacatepéquez'
    ),
    (
        '0315',
        'Sacatepéquez',
        'San Antonio Aguas Calientes',
        'San Antonio Aguas Calientes, Sacatepéquez'
    ),
    (
        '0316',
        'Sacatepéquez',
        'Santa Catarina Barahona',
        'Santa Catarina Barahona, Sacatepéquez'
    ),
    (
        '0401',
        'Chimaltenango',
        'Chimaltenango',
        'Chimaltenango, Chimaltenango'
    ),
    (
        '0402',
        'Chimaltenango',
        'San José Poaquil',
        'San José Poaquil, Chimaltenango'
    ),
    (
        '0403',
        'Chimaltenango',
        'San Martín Jilotepeque',
        'San Martín Jilotepeque, Chimaltenango'
    ),
    (
        '0404',
        'Chimaltenango',
        'San Juan Comalapa',
        'San Juan Comalapa, Chimaltenango'
    ),
    (
        '0405',
        'Chimaltenango',
        'Santa Apolonia',
        'Santa Apolonia, Chimaltenango'
    ),
    (
        '0406',
        'Chimaltenango',
        'Tecpán Guatemala',
        'Tecpán Guatemala, Chimaltenango'
    ),
    (
        '0407',
        'Chimaltenango',
        'Patzún',
        'Patzún, Chimaltenango'
    ),
    (
        '0408',
        'Chimaltenango',
        'San Miguel Pochuta',
        'San Miguel Pochuta, Chimaltenango'
    ),
    (
        '0409',
        'Chimaltenango',
        'Patzicía',
        'Patzicía, Chimaltenango'
    ),
    (
        '0410',
        'Chimaltenango',
        'Santa Cruz Balanyá',
        'Santa Cruz Balanyá, Chimaltenango'
    ),
    (
        '0411',
        'Chimaltenango',
        'Acatenango',
        'Acatenango, Chimaltenango'
    ),
    (
        '0412',
        'Chimaltenango',
        'San Pedro Yepocapa',
        'San Pedro Yepocapa, Chimaltenango'
    ),
    (
        '0413',
        'Chimaltenango',
        'San Andrés Itzapa',
        'San Andrés Itzapa, Chimaltenango'
    ),
    (
        '0414',
        'Chimaltenango',
        'Parramos',
        'Parramos, Chimaltenango'
    ),
    (
        '0415',
        'Chimaltenango',
        'Zaragoza',
        'Zaragoza, Chimaltenango'
    ),
    (
        '0416',
        'Chimaltenango',
        'El Tejar',
        'El Tejar, Chimaltenango'
    ),
    (
        '0501',
        'Escuintla',
        'Escuintla',
        'Escuintla, Escuintla'
    ),
    (
        '0502',
        'Escuintla',
        'Santa Lucía Cotzumalguapa',
        'Santa Lucía Cotzumalguapa, Escuintla'
    ),
    (
        '0503',
        'Escuintla',
        'La Democracia',
        'La Democracia, Escuintla'
    ),
    (
        '0504',
        'Escuintla',
        'Siquinalá',
        'Siquinalá, Escuintla'
    ),
    (
        '0505',
        'Escuintla',
        'Masagua',
        'Masagua, Escuintla'
    ),
    (
        '0506',
        'Escuintla',
        'Tiquisate',
        'Tiquisate, Escuintla'
    ),
    (
        '0507',
        'Escuintla',
        'La Gomera',
        'La Gomera, Escuintla'
    ),
    (
        '0508',
        'Escuintla',
        'Guanagazapa',
        'Guanagazapa, Escuintla'
    ),
    (
        '0509',
        'Escuintla',
        'San José',
        'San José, Escuintla'
    ),
    (
        '0510',
        'Escuintla',
        'Iztapa',
        'Iztapa, Escuintla'
    ),
    (
        '0511',
        'Escuintla',
        'Palín',
        'Palín, Escuintla'
    ),
    (
        '0512',
        'Escuintla',
        'San Vicente Pacaya',
        'San Vicente Pacaya, Escuintla'
    ),
    (
        '0513',
        'Escuintla',
        'Nueva Concepción',
        'Nueva Concepción, Escuintla'
    ),
    (
        '0514',
        'Escuintla',
        'Sipacate',
        'Sipacate, Escuintla'
    ),
    (
        '0601',
        'Santa Rosa',
        'Cuilapa',
        'Cuilapa, Santa Rosa'
    ),
    (
        '0602',
        'Santa Rosa',
        'Barberena',
        'Barberena, Santa Rosa'
    ),
    (
        '0603',
        'Santa Rosa',
        'Santa Rosa de Lima',
        'Santa Rosa de Lima, Santa Rosa'
    ),
    (
        '0604',
        'Santa Rosa',
        'Casillas',
        'Casillas, Santa Rosa'
    ),
    (
        '0605',
        'Santa Rosa',
        'San Rafael Las Flores',
        'San Rafael Las Flores, Santa Rosa'
    ),
    (
        '0606',
        'Santa Rosa',
        'Oratorio',
        'Oratorio, Santa Rosa'
    ),
    (
        '0607',
        'Santa Rosa',
        'San Juan Tecuaco',
        'San Juan Tecuaco, Santa Rosa'
    ),
    (
        '0608',
        'Santa Rosa',
        'Chiquimulilla',
        'Chiquimulilla, Santa Rosa'
    ),
    (
        '0609',
        'Santa Rosa',
        'Taxisco',
        'Taxisco, Santa Rosa'
    ),
    (
        '0610',
        'Santa Rosa',
        'Santa María Ixhuatán',
        'Santa María Ixhuatán, Santa Rosa'
    ),
    (
        '0611',
        'Santa Rosa',
        'Guazacapán',
        'Guazacapán, Santa Rosa'
    ),
    (
        '0612',
        'Santa Rosa',
        'Santa Cruz Naranjo',
        'Santa Cruz Naranjo, Santa Rosa'
    ),
    (
        '0613',
        'Santa Rosa',
        'Pueblo Nuevo Viñas',
        'Pueblo Nuevo Viñas, Santa Rosa'
    ),
    (
        '0701',
        'Sololá',
        'Sololá',
        'Sololá, Sololá'
    ),
    (
        '0702',
        'Sololá',
        'San José Chacayá',
        'San José Chacayá, Sololá'
    ),
    (
        '0703',
        'Sololá',
        'Santa María Visitación',
        'Santa María Visitación, Sololá'
    ),
    (
        '0704',
        'Sololá',
        'Santa Lucía Utatlán',
        'Santa Lucía Utatlán, Sololá'
    ),
    (
        '0705',
        'Sololá',
        'Nahualá',
        'Nahualá, Sololá'
    ),
    (
        '0706',
        'Sololá',
        'Santa Catarina Ixtahuacan',
        'Santa Catarina Ixtahuacan, Sololá'
    ),
    (
        '0707',
        'Sololá',
        'Santa Clara La Laguna',
        'Santa Clara La Laguna, Sololá'
    ),
    (
        '0708',
        'Sololá',
        'Concepción',
        'Concepción, Sololá'
    ),
    (
        '0709',
        'Sololá',
        'San Andrés Semetabaj',
        'San Andrés Semetabaj, Sololá'
    ),
    (
        '0710',
        'Sololá',
        'Panajachel',
        'Panajachel, Sololá'
    ),
    (
        '0711',
        'Sololá',
        'Santa Catarina Palopó',
        'Santa Catarina Palopó, Sololá'
    ),
    (
        '0712',
        'Sololá',
        'San Antonio Palopó',
        'San Antonio Palopó, Sololá'
    ),
    (
        '0713',
        'Sololá',
        'San Lucas Tolimán',
        'San Lucas Tolimán, Sololá'
    ),
    (
        '0714',
        'Sololá',
        'Santa Cruz La Laguna',
        'Santa Cruz La Laguna, Sololá'
    ),
    (
        '0715',
        'Sololá',
        'San Pablo La Laguna',
        'San Pablo La Laguna, Sololá'
    ),
    (
        '0716',
        'Sololá',
        'San Marcos La Laguna',
        'San Marcos La Laguna, Sololá'
    ),
    (
        '0717',
        'Sololá',
        'San Juan La Laguna',
        'San Juan La Laguna, Sololá'
    ),
    (
        '0718',
        'Sololá',
        'San Pedro La Laguna',
        'San Pedro La Laguna, Sololá'
    ),
    (
        '0719',
        'Sololá',
        'Santiago Atitlán',
        'Santiago Atitlán, Sololá'
    ),
    (
        '0801',
        'Totonicapán',
        'Totonicapán',
        'Totonicapán, Totonicapán'
    ),
    (
        '0802',
        'Totonicapán',
        'San Cristóbal Totonicapán',
        'San Cristóbal Totonicapán, Totonicapán'
    ),
    (
        '0803',
        'Totonicapán',
        'San Francisco El Alto',
        'San Francisco El Alto, Totonicapán'
    ),
    (
        '0804',
        'Totonicapán',
        'San Andrés Xecul',
        'San Andrés Xecul, Totonicapán'
    ),
    (
        '0805',
        'Totonicapán',
        'Momostenango',
        'Momostenango, Totonicapán'
    ),
    (
        '0806',
        'Totonicapán',
        'Santa María Chiquimula',
        'Santa María Chiquimula, Totonicapán'
    ),
    (
        '0807',
        'Totonicapán',
        'Santa Lucía La Reforma',
        'Santa Lucía La Reforma, Totonicapán'
    ),
    (
        '0808',
        'Totonicapán',
        'San Bartolo Aguas Calientes',
        'San Bartolo Aguas Calientes, Totonicapán'
    ),
    (
        '0901',
        'Quetzaltenango',
        'Quetzaltenango',
        'Quetzaltenango, Quetzaltenango'
    ),
    (
        '0902',
        'Quetzaltenango',
        'Salcajá',
        'Salcajá, Quetzaltenango'
    ),
    (
        '0903',
        'Quetzaltenango',
        'Olintepeque',
        'Olintepeque, Quetzaltenango'
    ),
    (
        '0904',
        'Quetzaltenango',
        'San Carlos Sija',
        'San Carlos Sija, Quetzaltenango'
    ),
    (
        '0905',
        'Quetzaltenango',
        'Sibilia',
        'Sibilia, Quetzaltenango'
    ),
    (
        '0906',
        'Quetzaltenango',
        'Cabricán',
        'Cabricán, Quetzaltenango'
    ),
    (
        '0907',
        'Quetzaltenango',
        'Cajolá',
        'Cajolá, Quetzaltenango'
    ),
    (
        '0908',
        'Quetzaltenango',
        'San Miguel Siguilá',
        'San Miguel Siguilá, Quetzaltenango'
    ),
    (
        '0909',
        'Quetzaltenango',
        'Ostuncalco',
        'Ostuncalco, Quetzaltenango'
    ),
    (
        '0910',
        'Quetzaltenango',
        'San Mateo',
        'San Mateo, Quetzaltenango'
    ),
    (
        '0911',
        'Quetzaltenango',
        'Concepción Chiquirichapa',
        'Concepción Chiquirichapa, Quetzaltenango'
    ),
    (
        '0912',
        'Quetzaltenango',
        'San Martín Sacatepéquez',
        'San Martín Sacatepéquez, Quetzaltenango'
    ),
    (
        '0913',
        'Quetzaltenango',
        'Almolonga',
        'Almolonga, Quetzaltenango'
    ),
    (
        '0914',
        'Quetzaltenango',
        'Cantel',
        'Cantel, Quetzaltenango'
    ),
    (
        '0915',
        'Quetzaltenango',
        'Huitán',
        'Huitán, Quetzaltenango'
    ),
    (
        '0916',
        'Quetzaltenango',
        'Zunil',
        'Zunil, Quetzaltenango'
    ),
    (
        '0917',
        'Quetzaltenango',
        'Colomba',
        'Colomba, Quetzaltenango'
    ),
    (
        '0918',
        'Quetzaltenango',
        'San Francisco La Unión',
        'San Francisco La Unión, Quetzaltenango'
    ),
    (
        '0919',
        'Quetzaltenango',
        'El Palmar',
        'El Palmar, Quetzaltenango'
    ),
    (
        '0920',
        'Quetzaltenango',
        'Coatepeque',
        'Coatepeque, Quetzaltenango'
    ),
    (
        '0921',
        'Quetzaltenango',
        'Genova',
        'Genova, Quetzaltenango'
    ),
    (
        '1001',
        'Suchitepéquez',
        'Mazatenango',
        'Mazatenango, Suchitepéquez'
    ),
    (
        '1002',
        'Suchitepéquez',
        'Cuyotenango',
        'Cuyotenango, Suchitepéquez'
    ),
    (
        '1003',
        'Suchitepéquez',
        'San Francisco Zapotitlán',
        'San Francisco Zapotitlán, Suchitepéquez'
    ),
    (
        '1004',
        'Suchitepéquez',
        'San Bernardino',
        'San Bernardino, Suchitepéquez'
    ),
    (
        '1005',
        'Suchitepéquez',
        'San José El Ídolo',
        'San José El Ídolo, Suchitepéquez'
    ),
    (
        '1006',
        'Suchitepéquez',
        'Santo Domingo Suchitepéquez',
        'Santo Domingo Suchitepéquez, Suchitepéquez'
    ),
    (
        '1007',
        'Suchitepéquez',
        'San Lorenzo',
        'San Lorenzo, Suchitepéquez'
    ),
    (
        '1008',
        'Suchitepéquez',
        'Samayac',
        'Samayac, Suchitepéquez'
    ),
    (
        '1009',
        'Suchitepéquez',
        'San Pablo Jocopilas',
        'San Pablo Jocopilas, Suchitepéquez'
    ),
    (
        '1010',
        'Suchitepéquez',
        'San Antonio Suchitepéquez',
        'San Antonio Suchitepéquez, Suchitepéquez'
    ),
    (
        '1011',
        'Suchitepéquez',
        'San Miguel Panán',
        'San Miguel Panán, Suchitepéquez'
    ),
    (
        '1012',
        'Suchitepéquez',
        'San Gabriel',
        'San Gabriel, Suchitepéquez'
    ),
    (
        '1013',
        'Suchitepéquez',
        'Chicacao',
        'Chicacao, Suchitepéquez'
    ),
    (
        '1014',
        'Suchitepéquez',
        'Patulul',
        'Patulul, Suchitepéquez'
    ),
    (
        '1015',
        'Suchitepéquez',
        'Santa Bárbara',
        'Santa Bárbara, Suchitepéquez'
    ),
    (
        '1016',
        'Suchitepéquez',
        'San Juan Bautista',
        'San Juan Bautista, Suchitepéquez'
    ),
    (
        '1017',
        'Suchitepéquez',
        'Santo Tomás La Unión',
        'Santo Tomás La Unión, Suchitepéquez'
    ),
    (
        '1018',
        'Suchitepéquez',
        'Zunilito',
        'Zunilito, Suchitepéquez'
    ),
    (
        '1019',
        'Suchitepéquez',
        'Pueblo Nuevo',
        'Pueblo Nuevo, Suchitepéquez'
    ),
    (
        '1101',
        'Retalhuleu',
        'Retalhuleu',
        'Retalhuleu, Retalhuleu'
    ),
    (
        '1102',
        'Retalhuleu',
        'San Sebastián',
        'San Sebastián, Retalhuleu'
    ),
    (
        '1103',
        'Retalhuleu',
        'Santa Cruz Muluá',
        'Santa Cruz Muluá, Retalhuleu'
    ),
    (
        '1104',
        'Retalhuleu',
        'San Martín Zapotitlán',
        'San Martín Zapotitlán, Retalhuleu'
    ),
    (
        '1105',
        'Retalhuleu',
        'San Felipe',
        'San Felipe, Retalhuleu'
    ),
    (
        '1106',
        'Retalhuleu',
        'San Andrés Villa Seca',
        'San Andrés Villa Seca, Retalhuleu'
    ),
    (
        '1107',
        'Retalhuleu',
        'Champerico',
        'Champerico, Retalhuleu'
    ),
    (
        '1108',
        'Retalhuleu',
        'Nuevo San Carlos',
        'Nuevo San Carlos, Retalhuleu'
    ),
    (
        '1201',
        'San Marcos',
        'San Marcos',
        'San Marcos, San Marcos'
    ),
    (
        '1202',
        'San Marcos',
        'San Pedro Sacatepéquez',
        'San Pedro Sacatepéquez, San Marcos'
    ),
    (
        '1203',
        'San Marcos',
        'San Antonio Sacatepéquez',
        'San Antonio Sacatepéquez, San Marcos'
    ),
    (
        '1204',
        'San Marcos',
        'Comitancillo',
        'Comitancillo, San Marcos'
    ),
    (
        '1205',
        'San Marcos',
        'San Miguel Ixtahuacán',
        'San Miguel Ixtahuacán, San Marcos'
    ),
    (
        '1206',
        'San Marcos',
        'Concepción Tutuapa',
        'Concepción Tutuapa, San Marcos'
    ),
    (
        '1207',
        'San Marcos',
        'Tacaná',
        'Tacaná, San Marcos'
    ),
    (
        '1208',
        'San Marcos',
        'Sibinal',
        'Sibinal, San Marcos'
    ),
    (
        '1209',
        'San Marcos',
        'Tajumulco',
        'Tajumulco, San Marcos'
    ),
    (
        '1210',
        'San Marcos',
        'Tejutla',
        'Tejutla, San Marcos'
    ),
    (
        '1211',
        'San Marcos',
        'San Rafael Pie de la Cuesta',
        'San Rafael Pie de la Cuesta, San Marcos'
    ),
    (
        '1212',
        'San Marcos',
        'Nuevo Progreso',
        'Nuevo Progreso, San Marcos'
    ),
    (
        '1213',
        'San Marcos',
        'El Tumbador',
        'El Tumbador, San Marcos'
    ),
    (
        '1214',
        'San Marcos',
        'El Rodeo',
        'El Rodeo, San Marcos'
    ),
    (
        '1215',
        'San Marcos',
        'Malacatán',
        'Malacatán, San Marcos'
    ),
    (
        '1216',
        'San Marcos',
        'Catarina',
        'Catarina, San Marcos'
    ),
    (
        '1217',
        'San Marcos',
        'Ayutla',
        'Ayutla, San Marcos'
    ),
    (
        '1218',
        'San Marcos',
        'Ocos',
        'Ocos, San Marcos'
    ),
    (
        '1219',
        'San Marcos',
        'San Pablo',
        'San Pablo, San Marcos'
    ),
    (
        '1220',
        'San Marcos',
        'El Quetzal',
        'El Quetzal, San Marcos'
    ),
    (
        '1221',
        'San Marcos',
        'La Reforma',
        'La Reforma, San Marcos'
    ),
    (
        '1222',
        'San Marcos',
        'Pajapita',
        'Pajapita, San Marcos'
    ),
    (
        '1223',
        'San Marcos',
        'Ixchiguán',
        'Ixchiguán, San Marcos'
    ),
    (
        '1301',
        'Huehuetenango',
        'Huehuetenango',
        'Huehuetenango, Huehuetenango'
    ),
    (
        '1302',
        'Huehuetenango',
        'Chiantla',
        'Chiantla, Huehuetenango'
    ),
    (
        '1303',
        'Huehuetenango',
        'Malacatancito',
        'Malacatancito, Huehuetenango'
    ),
    (
        '1304',
        'Huehuetenango',
        'Cuilco',
        'Cuilco, Huehuetenango'
    ),
    (
        '1305',
        'Huehuetenango',
        'Nentón',
        'Nentón, Huehuetenango'
    ),
    (
        '1306',
        'Huehuetenango',
        'San Pedro Necta',
        'San Pedro Necta, Huehuetenango'
    ),
    (
        '1307',
        'Huehuetenango',
        'Jacaltenango',
        'Jacaltenango, Huehuetenango'
    ),
    (
        '1308',
        'Huehuetenango',
        'Soloma',
        'Soloma, Huehuetenango'
    ),
    (
        '1309',
        'Huehuetenango',
        'Ixtahuacán',
        'Ixtahuacán, Huehuetenango'
    ),
    (
        '1310',
        'Huehuetenango',
        'Santa Bárbara',
        'Santa Bárbara, Huehuetenango'
    ),
    (
        '1311',
        'Huehuetenango',
        'La Libertad',
        'La Libertad, Huehuetenango'
    ),
    (
        '1312',
        'Huehuetenango',
        'La Democracia',
        'La Democracia, Huehuetenango'
    ),
    (
        '1313',
        'Huehuetenango',
        'San Miguel Acatán',
        'San Miguel Acatán, Huehuetenango'
    ),
    (
        '1314',
        'Huehuetenango',
        'San Rafael La Independencia',
        'San Rafael La Independencia, Huehuetenango'
    ),
    (
        '1315',
        'Huehuetenango',
        'Todos Santos Cuchumatán',
        'Todos Santos Cuchumatán, Huehuetenango'
    ),
    (
        '1316',
        'Huehuetenango',
        'San Juan Atitán',
        'San Juan Atitán, Huehuetenango'
    ),
    (
        '1317',
        'Huehuetenango',
        'Santa Eulalia',
        'Santa Eulalia, Huehuetenango'
    ),
    (
        '1318',
        'Huehuetenango',
        'San Mateo Ixtatán',
        'San Mateo Ixtatán, Huehuetenango'
    ),
    (
        '1319',
        'Huehuetenango',
        'Colotenango',
        'Colotenango, Huehuetenango'
    ),
    (
        '1320',
        'Huehuetenango',
        'San Sebastián Coatán',
        'San Sebastián Coatán, Huehuetenango'
    ),
    (
        '1321',
        'Huehuetenango',
        'Concepción Huista',
        'Concepción Huista, Huehuetenango'
    ),
    (
        '1322',
        'Huehuetenango',
        'San Juan Ixcoy',
        'San Juan Ixcoy, Huehuetenango'
    ),
    (
        '1323',
        'Huehuetenango',
        'San Antonio Huista',
        'San Antonio Huista, Huehuetenango'
    ),
    (
        '1324',
        'Huehuetenango',
        'San Sebastián Huehuetenango',
        'San Sebastián Huehuetenango, Huehuetenango'
    ),
    (
        '1325',
        'Huehuetenango',
        'Santa Cruz Barillas',
        'Santa Cruz Barillas, Huehuetenango'
    ),
    (
        '1326',
        'Huehuetenango',
        'San Gaspar Ixchil',
        'San Gaspar Ixchil, Huehuetenango'
    ),
    (
        '1327',
        'Huehuetenango',
        'Aguacatán',
        'Aguacatán, Huehuetenango'
    ),
    (
        '1328',
        'Huehuetenango',
        'San Rafael Pétzal',
        'San Rafael Pétzal, Huehuetenango'
    ),
    (
        '1329',
        'Huehuetenango',
        'San Gaspar Ixchil',
        'San Gaspar Ixchil, Huehuetenango'
    ),
    (
        '1330',
        'Huehuetenango',
        'Santiago Chimaltenango',
        'Santiago Chimaltenango, Huehuetenango'
    ),
    (
        '1331',
        'Huehuetenango',
        'Santa Ana Huista',
        'Santa Ana Huista, Huehuetenango'
    ),
    (
        '1401',
        'Quiché',
        'Santa Cruz del Quiché',
        'Santa Cruz del Quiché, Quiché'
    ),
    (
        '1402',
        'Quiché',
        'Chiché',
        'Chiché, Quiché'
    ),
    (
        '1403',
        'Quiché',
        'Chinique',
        'Chinique, Quiché'
    ),
    (
        '1404',
        'Quiché',
        'Zacualpa',
        'Zacualpa, Quiché'
    ),
    (
        '1405',
        'Quiché',
        'Chajul',
        'Chajul, Quiché'
    ),
    (
        '1406',
        'Quiché',
        'Chichicastenango',
        'Chichicastenango, Quiché'
    ),
    (
        '1407',
        'Quiché',
        'Patzité',
        'Patzité, Quiché'
    ),
    (
        '1408',
        'Quiché',
        'San Antonio Ilotenango',
        'San Antonio Ilotenango, Quiché'
    ),
    (
        '1409',
        'Quiché',
        'San Pedro Jocopilas',
        'San Pedro Jocopilas, Quiché'
    ),
    (
        '1410',
        'Quiché',
        'Cunén',
        'Cunén, Quiché'
    ),
    (
        '1411',
        'Quiché',
        'San Juan Cotzal',
        'San Juan Cotzal, Quiché'
    ),
    (
        '1412',
        'Quiché',
        'Joyabaj',
        'Joyabaj, Quiché'
    ),
    (
        '1413',
        'Quiché',
        'Santa María Nebaj',
        'Santa María Nebaj, Quiché'
    ),
    (
        '1414',
        'Quiché',
        'San Andrés Sajcabajá',
        'San Andrés Sajcabajá, Quiché'
    ),
    (
        '1415',
        'Quiché',
        'San Miguel Uspantán',
        'San Miguel Uspantán, Quiché'
    ),
    (
        '1416',
        'Quiché',
        'Sacapulas',
        'Sacapulas, Quiché'
    ),
    (
        '1417',
        'Quiché',
        'San Bartolomé Jocotenango',
        'San Bartolomé Jocotenango, Quiché'
    ),
    (
        '1418',
        'Quiché',
        'Canillá',
        'Canillá, Quiché'
    ),
    (
        '1419',
        'Quiché',
        'Chicamán',
        'Chicamán, Quiché'
    ),
    (
        '1420',
        'Quiché',
        'Playa Grande Ixcán',
        'Playa Grande Ixcán, Quiché'
    ),
    (
        '1421',
        'Quiché',
        'Pachalum',
        'Pachalum, Quiché'
    ),
    (
        '1501',
        'Baja Verapaz',
        'Salamá',
        'Salamá, Baja Verapaz'
    ),
    (
        '1502',
        'Baja Verapaz',
        'San Miguel Chicaj',
        'San Miguel Chicaj, Baja Verapaz'
    ),
    (
        '1503',
        'Baja Verapaz',
        'Rabinal',
        'Rabinal, Baja Verapaz'
    ),
    (
        '1504',
        'Baja Verapaz',
        'Cubulco',
        'Cubulco, Baja Verapaz'
    ),
    (
        '1505',
        'Baja Verapaz',
        'Granados',
        'Granados, Baja Verapaz'
    ),
    (
        '1506',
        'Baja Verapaz',
        'Santa Cruz El Chol',
        'Santa Cruz El Chol, Baja Verapaz'
    ),
    (
        '1507',
        'Baja Verapaz',
        'San Jerónimo',
        'San Jerónimo, Baja Verapaz'
    ),
    (
        '1508',
        'Baja Verapaz',
        'Purulhá',
        'Purulhá, Baja Verapaz'
    ),
    (
        '1601',
        'Alta Verapaz',
        'Cobán',
        'Cobán, Alta Verapaz'
    ),
    (
        '1602',
        'Alta Verapaz',
        'Santa Cruz Verapaz',
        'Santa Cruz Verapaz, Alta Verapaz'
    ),
    (
        '1603',
        'Alta Verapaz',
        'San Cristóbal Verapaz',
        'San Cristóbal Verapaz, Alta Verapaz'
    ),
    (
        '1604',
        'Alta Verapaz',
        'Tactic',
        'Tactic, Alta Verapaz'
    ),
    (
        '1605',
        'Alta Verapaz',
        'Tamahú',
        'Tamahú, Alta Verapaz'
    ),
    (
        '1606',
        'Alta Verapaz',
        'San Miguel Tucurú',
        'San Miguel Tucurú, Alta Verapaz'
    ),
    (
        '1607',
        'Alta Verapaz',
        'Panzós',
        'Panzós, Alta Verapaz'
    ),
    (
        '1608',
        'Alta Verapaz',
        'Senahú',
        'Senahú, Alta Verapaz'
    ),
    (
        '1609',
        'Alta Verapaz',
        'San Pedro Carchá',
        'San Pedro Carchá, Alta Verapaz'
    ),
    (
        '1610',
        'Alta Verapaz',
        'San Juan Chamelco',
        'San Juan Chamelco, Alta Verapaz'
    ),
    (
        '1611',
        'Alta Verapaz',
        'San Agustín Lanquín',
        'San Agustín Lanquín, Alta Verapaz'
    ),
    (
        '1612',
        'Alta Verapaz',
        'Santa María Cahabón',
        'Santa María Cahabón, Alta Verapaz'
    ),
    (
        '1613',
        'Alta Verapaz',
        'Chisec',
        'Chisec, Alta Verapaz'
    ),
    (
        '1614',
        'Alta Verapaz',
        'Chahal',
        'Chahal, Alta Verapaz'
    ),
    (
        '1615',
        'Alta Verapaz',
        'Fray Bartolomé de Las Casas',
        'Fray Bartolomé de Las Casas, Alta Verapaz'
    ),
    (
        '1616',
        'Alta Verapaz',
        'Santa Catalina La Tinta',
        'Santa Catalina La Tinta, Alta Verapaz'
    ),
    (
        '1617',
        'Alta Verapaz',
        'Raxruhá',
        'Raxruhá, Alta Verapaz'
    ),
    (
        '1701',
        'Petén',
        'Flores',
        'Flores, Petén'
    ),
    (
        '1702',
        'Petén',
        'San José',
        'San José, Petén'
    ),
    (
        '1703',
        'Petén',
        'San Benito',
        'San Benito, Petén'
    ),
    (
        '1704',
        'Petén',
        'San Andrés',
        'San Andrés, Petén'
    ),
    (
        '1705',
        'Petén',
        'La Libertad',
        'La Libertad, Petén'
    ),
    (
        '1706',
        'Petén',
        'San Francisco',
        'San Francisco, Petén'
    ),
    (
        '1707',
        'Petén',
        'Santa Ana',
        'Santa Ana, Petén'
    ),
    (
        '1708',
        'Petén',
        'Dolores',
        'Dolores, Petén'
    ),
    (
        '1709',
        'Petén',
        'San Luis',
        'San Luis, Petén'
    ),
    (
        '1710',
        'Petén',
        'Sayaxché',
        'Sayaxché, Petén'
    ),
    (
        '1711',
        'Petén',
        'Melchor de Mencos',
        'Melchor de Mencos, Petén'
    ),
    (
        '1712',
        'Petén',
        'Poptún',
        'Poptún, Petén'
    ),
    (
        '1713',
        'Petén',
        'Las Cruces',
        'Las Cruces, Petén'
    ),
    (
        '1714',
        'Petén',
        'El Chal',
        'El Chal, Petén'
    ),
    (
        '1801',
        'Izabal',
        'Puerto Barrios',
        'Puerto Barrios, Izabal'
    ),
    (
        '1802',
        'Izabal',
        'Livingston',
        'Livingston, Izabal'
    ),
    (
        '1803',
        'Izabal',
        'El Estor',
        'El Estor, Izabal'
    ),
    (
        '1804',
        'Izabal',
        'Morales',
        'Morales, Izabal'
    ),
    (
        '1805',
        'Izabal',
        'Los Amates',
        'Los Amates, Izabal'
    ),
    (
        '1901',
        'Zacapa',
        'Zacapa',
        'Zacapa, Zacapa'
    ),
    (
        '1902',
        'Zacapa',
        'Estanzuela',
        'Estanzuela, Zacapa'
    ),
    (
        '1903',
        'Zacapa',
        'Río Hondo',
        'Río Hondo, Zacapa'
    ),
    (
        '1904',
        'Zacapa',
        'Gualán',
        'Gualán, Zacapa'
    ),
    (
        '1905',
        'Zacapa',
        'Teculután',
        'Teculután, Zacapa'
    ),
    (
        '1906',
        'Zacapa',
        'Usumatlán',
        'Usumatlán, Zacapa'
    ),
    (
        '1907',
        'Zacapa',
        'Cabañas',
        'Cabañas, Zacapa'
    ),
    (
        '1908',
        'Zacapa',
        'San Diego',
        'San Diego, Zacapa'
    ),
    (
        '1909',
        'Zacapa',
        'La Unión',
        'La Unión, Zacapa'
    ),
    (
        '1910',
        'Zacapa',
        'Huité',
        'Huité, Zacapa'
    ),
    (
        '1911',
        'Zacapa',
        'San Jorge',
        'San Jorge, Zacapa'
    ),
    (
        '2001',
        'Chiquimula',
        'Chiquimula',
        'Chiquimula, Chiquimula'
    ),
    (
        '2002',
        'Chiquimula',
        'San José La Arada',
        'San José La Arada, Chiquimula'
    ),
    (
        '2003',
        'Chiquimula',
        'San Juan Ermita',
        'San Juan Ermita, Chiquimula'
    ),
    (
        '2004',
        'Chiquimula',
        'Jocotán Chiquimula',
        'Jocotán Chiquimula, Chiquimula'
    ),
    (
        '2005',
        'Chiquimula',
        'Camotán Chiquimula',
        'Camotán Chiquimula, Chiquimula'
    ),
    (
        '2006',
        'Chiquimula',
        'Olopa Chiquimula',
        'Olopa Chiquimula, Chiquimula'
    ),
    (
        '2007',
        'Chiquimula',
        'Esquipulas',
        'Esquipulas, Chiquimula'
    ),
    (
        '2008',
        'Chiquimula',
        'Concepción Las Minas Chiquimula',
        'Concepción Las Minas Chiquimula, Chiquimula'
    ),
    (
        '2009',
        'Chiquimula',
        'Quezaltepeque Chiquimula',
        'Quezaltepeque Chiquimula, Chiquimula'
    ),
    (
        '2010',
        'Chiquimula',
        'San Jacinto Chiquimula',
        'San Jacinto Chiquimula, Chiquimula'
    ),
    (
        '2011',
        'Chiquimula',
        'Ipala Chiquimula',
        'Ipala Chiquimula, Chiquimula'
    ),
    (
        '2101',
        'Jalapa',
        'Jalapa',
        'Jalapa, Jalapa'
    ),
    (
        '2102',
        'Jalapa',
        'San Pedro Pinula',
        'San Pedro Pinula, Jalapa'
    ),
    (
        '2103',
        'Jalapa',
        'San Luis Jilotepeque',
        'San Luis Jilotepeque, Jalapa'
    ),
    (
        '2104',
        'Jalapa',
        'San Manuel Chaparrón',
        'San Manuel Chaparrón, Jalapa'
    ),
    (
        '2105',
        'Jalapa',
        'San Carlos Alzatate',
        'San Carlos Alzatate, Jalapa'
    ),
    (
        '2106',
        'Jalapa',
        'Monjas',
        'Monjas, Jalapa'
    ),
    (
        '2107',
        'Jalapa',
        'Mataquescuintla',
        'Mataquescuintla, Jalapa'
    ),
    (
        '2201',
        'Jutiapa',
        'Jutiapa',
        'Jutiapa, Jutiapa'
    ),
    (
        '2202',
        'Jutiapa',
        'El Progreso',
        'El Progreso, Jutiapa'
    ),
    (
        '2203',
        'Jutiapa',
        'Santa Catarina Mita',
        'Santa Catarina Mita, Jutiapa'
    ),
    (
        '2204',
        'Jutiapa',
        'Agua Blanca',
        'Agua Blanca, Jutiapa'
    ),
    (
        '2205',
        'Jutiapa',
        'Asunción Mita',
        'Asunción Mita, Jutiapa'
    ),
    (
        '2206',
        'Jutiapa',
        'Yupiltepeque',
        'Yupiltepeque, Jutiapa'
    ),
    (
        '2207',
        'Jutiapa',
        'Atescatempa',
        'Atescatempa, Jutiapa'
    ),
    (
        '2208',
        'Jutiapa',
        'Jerez',
        'Jerez, Jutiapa'
    ),
    (
        '2209',
        'Jutiapa',
        'El Adelanto',
        'El Adelanto, Jutiapa'
    ),
    (
        '2210',
        'Jutiapa',
        'Zapotitlán',
        'Zapotitlán, Jutiapa'
    ),
    (
        '2211',
        'Jutiapa',
        'Comapa',
        'Comapa, Jutiapa'
    ),
    (
        '2212',
        'Jutiapa',
        'Jalpatagua',
        'Jalpatagua, Jutiapa'
    ),
    (
        '2213',
        'Jutiapa',
        'Conguaco',
        'Conguaco, Jutiapa'
    ),
    (
        '2214',
        'Jutiapa',
        'Moyuta',
        'Moyuta, Jutiapa'
    ),
    (
        '2215',
        'Jutiapa',
        'Pasaco',
        'Pasaco, Jutiapa'
    ),
    (
        '2216',
        'Jutiapa',
        'San José Acatempa',
        'San José Acatempa, Jutiapa'
    ),
    (
        '2217',
        'Jutiapa',
        'Quesada',
        'Quesada, Jutiapa'
    ),
    (
        '0009',
        'Otro Pais',
        'Otro Pais',
        'Otro Pais, Otro Pais'
    );

INSERT INTO
    especialidad (nombre, especialista)
VALUES (
        'Medicina Interna',
        'Médico Internista'
    ),
    (
        'Pediatria',
        'Médico Pediatra'
    ),
    (
        'Ginecologia y Obstetricia',
        'Médico Ginecólogo'
    ),
    ('Cirugia', 'Médico Cirujano'),
    (
        'Traumatologia',
        'Médico Traumatólogo'
    ),
    ('Psicologia', 'Psicóloga'),
    ('Nutrición', 'Nutricionista'),
    (
        'Medicina General',
        'Médico General'
    );

INSERT INTO
    parentescos (id, nombre)
VALUES (1, 'Madre/Padre'),
    (2, 'Hijo/a'),
    (3, 'Hermano/a'),
    (4, 'Abuelo/a'),
    (5, 'Tío/a'),
    (6, 'Primo/a'),
    (7, 'Sobrino/a'),
    (8, 'Yerno/Nuera'),
    (9, 'Esposo/a'),
    (10, 'Suegro/a'),
    (11, 'Tutor'),
    (12, 'Amistad'),
    (13, 'Novio/a'),
    (14, 'Cuñado/a'),
    (15, 'Nieto/a'),
    (16, 'Hijastros'),
    (17, 'Padrastros'),
    (18, 'Otro');

INSERT INTO
    pueblos (id, nombre)
VALUES (1, 'Ladino'),
    (2, 'Maya'),
    (3, 'Garífuna'),
    (4, 'Xinca'),
    (5, 'Otros'),
    (6, 'No indica');

INSERT INTO
    idiomas (id, nombre)
VALUES (1, 'Achi'),
    (2, 'Akateka'),
    (3, 'Awakateka'),
    (4, 'Chorti'),
    (5, 'Chalchiteka'),
    (6, 'Chuj'),
    (7, 'Itza'),
    (8, 'Ixil'),
    (9, 'Jakalteka'),
    (10, 'Kaqchikel'),
    (11, 'Kiche'),
    (12, 'Mam'),
    (13, 'Mopan'),
    (14, 'Poqomam'),
    (15, 'Pocomchi'),
    (16, 'Qanjobal'),
    (17, 'Qeqchi'),
    (18, 'Sakapulteka'),
    (19, 'Sipakapensa'),
    (20, 'Tektiteka'),
    (21, 'Tzutujil'),
    (22, 'Uspanteka'),
    (23, 'No indica'),
    (24, 'Español'),
    (25, 'Otro');

INSERT INTO
    estados_civiles (id, nombre)
VALUES (1, 'Casado'),
    (2, 'Unido'),
    (3, 'Soltero');

INSERT INTO
    educacion (id, nivel)
VALUES (1, 'No aplica'),
    (2, 'Pre Primaria'),
    (3, 'Primaria'),
    (4, 'Básicos'),
    (5, 'Diversificado'),
    (6, 'Universidad'),
    (7, 'Ninguno'),
    (8, 'Otro'),
    (9, 'No indica');

INSERT INTO
    tipos_parto (descripcion)
VALUES ('Simple'),
    ('Doble'),
    ('Multiple');

INSERT INTO
    clases_parto (descripcion)
VALUES ('Eutócico'),
    ('Distócico');

INSERT INTO
    servicios (nombre)
VALUES ('Emergencia'),
    ('SOP'),
    ('Maternidad'),
    ('Ginecologia'),
    ('Cirugia'),
    ('Cirugia pedia'),
    ('Trauma'),
    ('Trauma pedia'),
    ('CRN'),
    ('Pediatria'),
    ('Alojamiento RN'),
    ('Neonatos'),
    ('Medicina hombres'),
    ('Medicina mujeres'),
    ('vsvs'),
    ('Covid'),
    ('labor & parto'),
    ('area roja emergencia'),
    ('Medicina'),
    ('UCIN'),
    ('COEX');

INSERT INTO
    servio_es (descripcion)
VALUES ('COEX'),
    ('Encamamiento'),
    ('Emergencia'),
    ('SOP emergencia'),
    ('SOP electiva'),
    ('Maternidad');

INSERT INTO
    estados_salud (nombre)
VALUES ('Estable'),
    ('Delicado'),
    ('Fallecido');

INSERT INTO
    situaciones_salud (nombre)
VALUES ('Hospitalizado'),
    ('Observación'),
    ('Recuperado'),
    ('Referido'),
    ('Trasladado'),
    ('Fugado'),
    ('Fallecido');

INSERT INTO
    lugares_referencia (nombre)
VALUES (
        'Hospital General San Juan De Dios - Guatemala'
    ),
    (
        'Hospital Roosevelt - Guatemala'
    ),
    (
        'Hospital Villa Nueva - Guatemala'
    ),
    (
        'Hospital Regional De Amatitlán - Guatemala'
    ),
    (
        'Hospital Nacional Pedro De Bethancourt - Sacatepequez'
    ),
    (
        'Hospital Nacional Chimaltenango - Chimaltenango'
    ),
    (
        'Hospital Distrital De Tiquisate - Escuintla'
    ),
    (
        'Hospital Regional De Escuitla - Escuintla'
    ),
    (
        'Hospital Intregado El Progreso - El Progreso'
    ),
    (
        'Hospital Regional De Cuilapa - Santa Rosa'
    ),
    (
        'Hospital Nacional Juan De Dios Rodas - Sololá'
    ),
    (
        'Hospital Nacional Dr Jose Felipe Flores-  Totonicapán'
    ),
    (
        'Hospital Nacional Dr Juan Jose Ortega - Quetzaltenango'
    ),
    (
        'Hospital Regional De Occidente San Juan De Dios - Quetzaltenango'
    ),
    (
        'Hospital Antituberculoso Dr Rodolfo Robles - Quetzaltenango'
    ),
    (
        'Hospital Nacional De Mazatenango - Suchitepequez'
    ),
    (
        'Hospital Nacional de Retalhuleu - Retalhuleu'
    ),
    (
        'Hospital Distrital De Malacatán - San Marcos'
    ),
    (
        'Hospital Nacional De San Marcos - San Marcos'
    ),
    (
        'Hospital Distrital De Joyabaj - Quiché'
    ),
    (
        'Hospital Distrital Uspantán - Quiché'
    ),
    (
        'Hospital Nacional Santa Elena - Quiché'
    ),
    (
        'Hospital Nacional De Salamá - Baja Verapaz'
    ),
    (
        'Hospital Distrital Fray Bartolome De Las Casas - Alta Verapaz'
    ),
    (
        'Hospital Nacional Hellen Lossi De Laugerud - Alta Verapaz'
    ),
    (
        'Hospital Distrital La Tinta - Alta Verapaz'
    ),
    (
        'Hospital Regional Dr Antonio Penado - Petén'
    ),
    (
        'Hospital Distrital Melchor De Mencos - Petén'
    ),
    (
        'Hospital Distrital De Sayaxché - Petén'
    ),
    (
        'Hospital Distrital De Poptún - Petén'
    ),
    (
        'Hospital Infantil Elisa Martinez - Izabal'
    ),
    (
        'Hospital Nacional De La Amistad Guatemala Japon  - Izabal'
    ),
    (
        'Hospital Regional De Zacapa - Zacapa'
    ),
    (
        'Hospital Modular Carlos Arana Osorio - Chiquimula'
    ),
    (
        'Hospital Nacional Nicolasa Cruz - Jalapa'
    ),
    (
        'Hospital Nacional Ernestina Viuda De Recinos - Jutiapa'
    ),
    (
        'Hospital Distrital Nebaj - Quiché'
    ),
    (
        'Hospital Antituberculoso San Vicente - Guatemala'
    ),
    (
        'Hospital Infantil de Infectología y Rehabilitación - Guatemala'
    ),
    (
        'Hospital de Salud Mental Dr Federico Mora - Guatemala'
    ),
    (
        'Hospital de Ortopedia y Rehabilitación Dr Jorge V - Guatemala'
    ),
    ('IGSS - especificar en notas'),
    (
        'Privado - especificar en notas'
    );

INSERT INTO
    estadia_es (descripcion)
VALUES ('cama'),
    ('camilla'),
    ('silla de ruedas'),
    ('no aplica'),
    ('otro');

INSERT INTO
    tipo_consulta (descripcion)
VALUES ('Coex'),
    ('Hospitalización'),
    ('Emergencia');

INSERT INTO
    estudios (descripcion)
VALUES ('USG Obstetrico'),
    ('USG Pelvico'),
    ('Colposcopia'),
    ('Rayos X');

INSERT INTO
    tipo_citas (descripcion)
VALUES ('consulta'),
    ('ingreso - operatorio'),
    ('USG Obstetrico'),
    ('USG Pelvico'),
    ('Colposcopia'),
    ('especial'),
    ('con recordatorio');

INSERT INTO
    estatus (descripcion)
VALUES ('activo'),
    ('archivado'),
    ('prestamo'),
    ('desconocido');

INSERT INTO
    grupo_edad (
        grupo,
        edad_inicio,
        edad_fin,
        caracteristicas
    )
VALUES (
        "Neonato",
        0,
        0,
        "Adaptación a la vida fuera del útero"
    ),
    (
        "Lactante menor",
        1,
        1,
        "Desarrollo rápido, inicio de la locomoción y el lenguaje"
    ),
    (
        "Lactante mayor",
        1,
        2,
        "Desarrollo del caminar y mayor interacción social"
    ),
    (
        "Primera infancia",
        2,
        6,
        "Desarrollo de habilidades motoras, lenguaje y socialización"
    ),
    (
        "Infancia media",
        6,
        12,
        "Crecimiento continuo, avance cognitivo y socialización compleja"
    ),
    (
        "Adolescencia temprana",
        12,
        15,
        "Cambios hormonales y físicos significativos búsqueda de identidad"
    ),
    (
        "Adolescencia tardía",
        15,
        18,
        "Mayor madurez física y emocional transición a la adultez"
    ),
    (
        "Juventud",
        18,
        25,
        "Consolidación de independencia, educación o inicio laboral"
    ),
    (
        "Adultez temprana",
        25,
        40,
        "Plenitud física, estabilidad emocional y reproductiva"
    ),
    (
        "Adultez media",
        40,
        60,
        "Cambios progresivos por envejecimiento, estabilidad laboral y familiar"
    ),
    (
        "Adulto mayor joven",
        60,
        75,
        "Salud razonablemente buena, actividad moderada"
    ),
    (
        "Adulto mayor mayor",
        75,
        NULL,
        "Vulnerabilidad física y necesidad de cuidados específicos"
    );

INSERT INTO
    tipo_lesion (nombre, descripcion)
VALUES (
        'Quemadura',
        'Lesión producida por exposición al calor, fuego, sustancias químicas, electricidad o radiación.'
    ),
    (
        'Arma de fuego',
        'Lesión causada por proyectiles disparados por armas de fuego.'
    ),
    (
        'Arma blanca',
        'Lesión causada por objetos cortopunzantes como cuchillos, navajas o espadas.'
    ),
    (
        'Contusión',
        'Lesión producida por un golpe que no rompe la piel, pero daña tejidos internos.'
    ),
    (
        'Fractura',
        'Rotura o fisura de un hueso debido a un impacto o estrés.'
    ),
    (
        'Luxación',
        'Desplazamiento de un hueso de su posición normal en una articulación.'
    ),
    (
        'Corte',
        'Herida producida por un objeto afilado que rompe la piel.'
    ),
    (
        'Envenenamiento',
        'Lesión causada por la ingestión, inhalación o contacto con sustancias tóxicas.'
    ),
    (
        'Aplastamiento',
        'Lesión causada por la compresión extrema de un cuerpo o parte del cuerpo.'
    ),
    (
        'Electrocución',
        'Lesión causada por la exposición a una descarga eléctrica.'
    ),
    (
        'Caída de altura',
        'Lesión producida por una caída desde lugares altos como escaleras, edificios o árboles.'
    ),
    (
        'Caída al mismo nivel',
        'Lesión causada por resbalones, tropiezos o accidentes en superficies planas.'
    ),
    (
        'Accidente de tránsito',
        'Lesión causada por colisiones entre vehículos, motocicletas o bicicletas.'
    ),
    (
        'Atropellamiento',
        'Lesión sufrida al ser golpeado por un vehículo en movimiento.'
    ),
    (
        'Ahogamiento',
        'Lesión o muerte causada por la falta de oxígeno al estar sumergido en agua u otros líquidos.'
    ),
    (
        'Asfixia',
        'Falta de aire debido a obstrucción de vías respiratorias por objetos o sustancias.'
    ),
    (
        'Explosión',
        'Lesión causada por ondas expansivas, fragmentos o fuego debido a una explosión.'
    ),
    (
        'Mordedura de animal',
        'Lesión causada por el ataque de animales como perros, serpientes o insectos.'
    ),
    (
        'Picadura',
        'Lesión causada por insectos como abejas, avispas, arañas o escorpiones.'
    ),
    (
        'Golpe con objeto',
        'Lesión causada por el impacto de objetos como martillos, piedras o herramientas.'
    ),
    (
        'Accidente laboral',
        'Lesión producida por herramientas, maquinaria o condiciones laborales peligrosas.'
    ),
    (
        'Lesión deportiva',
        'Lesión causada durante la práctica de deportes, como desgarros musculares o fracturas.'
    );

INSERT INTO
    permisos (id, nombre, descripcion)
VALUES (
        1,
        'crear',
        'Permiso para crear'
    ),
    (
        2,
        'editar',
        'Permiso para editar'
    ),
    (
        3,
        'borrar',
        'Permiso para eliminar'
    ),
    (4, 'ver', 'Permiso para ver'),
    (
        5,
        'navegar',
        'Permiso para navegar en la aplicación'
    );

INSERT INTO
    roles (
        id,
        nombre,
        descripcion,
        permisos
    )
VALUES (
        1,
        'Administrador',
        'Acceso completo al sistema',
        'crear,editar,borrar,ver,navegar'
    ),
    (
        2,
        'estandar',
        'Puede ver, editar y navegar, pero no crear ni eliminar usuarios',
        'editar,ver,navegar'
    ),
    (
        3,
        'Observador',
        'Solo puede ver y navegar',
        'ver,navegar'
    ),
    (
        4,
        'registrador',
        'Puede ver, editar y navegar, pero no crear ni eliminar usuarios',
        'editar,ver,navegar'
    ),
    (
        5,
        'informacion',
        'Puede ver, editar y navegar, pero no crear ni eliminar usuarios',
        'editar,ver,navegar'
    );

INSERT INTO
    nacionalidades (
        iso,
        nacionalidad,
        pais,
        cti,
        idioma
    )
VALUES (
        'USA',
        'Estadounidense',
        'Estados Unidos',
        1,
        'Inglés'
    ),
    (
        'CAN',
        'Canadiense',
        'Canadá',
        1,
        'Inglés, Francés'
    ),
    (
        'MEX',
        'Mexicano',
        'México',
        52,
        'Español'
    ),
    (
        'BLZ',
        'Beliceño',
        'Belice',
        501,
        'Inglés'
    ),
    (
        'CRI',
        'Costarricense',
        'Costa Rica',
        506,
        'Español'
    ),
    (
        'SLV',
        'Salvadoreño',
        'El Salvador',
        503,
        'Español'
    ),
    (
        'GTM',
        'Guatemalteco',
        'Guatemala',
        502,
        'Español'
    ),
    (
        'HND',
        'Hondureño',
        'Honduras',
        504,
        'Español'
    ),
    (
        'NIC',
        'Nicaragüense',
        'Nicaragua',
        505,
        'Español'
    ),
    (
        'PAN',
        'Panameño',
        'Panamá',
        507,
        'Español'
    ),
    (
        'DOM',
        'Dominicano',
        'República Dominicana',
        1,
        'Español'
    ),
    (
        'COL',
        'Colombiano',
        'Colombia',
        57,
        'Español'
    ),
    (
        'ECU',
        'Ecuatoriano',
        'Ecuador',
        593,
        'Español'
    ),
    (
        'PER',
        'Peruano',
        'Perú',
        51,
        'Español'
    ),
    (
        'VEN',
        'Venezolano',
        'Venezuela',
        58,
        'Español'
    ),
    (
        'BOL',
        'Boliviano',
        'Bolivia',
        591,
        'Español, Quechua, Aymara'
    ),
    (
        'PRY',
        'Paraguayo',
        'Paraguay',
        595,
        'Español, Guaraní'
    ),
    (
        'CHL',
        'Chileno',
        'Chile',
        56,
        'Español'
    ),
    (
        'ARG',
        'Argentino',
        'Argentina',
        54,
        'Español'
    ),
    (
        'URY',
        'Uruguayo',
        'Uruguay',
        598,
        'Español'
    ),
    (
        'BRA',
        'Brasileño',
        'Brasil',
        55,
        'Portugués'
    ),
    (
        'SUR',
        'Surinamés',
        'Surinam',
        597,
        'Neerlandés'
    ),
    (
        'FRA',
        'Francés',
        'Francia Guayana',
        594,
        'Francés'
    ),
    (
        'GUY',
        'Guyano',
        'Guyana Francesa',
        594,
        'Francés'
    );